...
public abstract class UuidConverter {
	// Varies from standard Base64 by the last two characters in this string
	// ("-" and "_").
	// The standard characters are "+" and "/" respectively, but are not URL
	// safe.
	private static final char[] C64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"
			.toCharArray();
	 ...
	 *             if the UUID instance is null.
	 * @throws IllegalArgumentException
	 *             if the underlying UUID implementation is not 16 bytes.
	 */
	...
	/**
	 * Given a UUID representation (either a short or long form), return a UUID
	 * from it.
	 * <p/>
	 * If the uuidString is longer than our short, 22-character form (or 24 with
	 * padding), it is assumed to be a full-length 36-character UUID string.
	 * 
	 * @param uuidString
	...
	public static UUID parse(String uuidString) {
		...
		if (uuidString.length() < 22) {
			throw new IllegalArgumentException(
					"Short UUID must be 22 characters: " + uuidString);
		}
	...
	/**
	 * Accepts a UUID byte array (of exactly 16 bytes) and base64 encodes it,
	 * using a URL-safe encoding scheme. The resulting string will be 22
	 * characters in length with no extra padding on the end (e.g. no "==" on
	 * the end).
	 * <p/>
	 * Base64 encoding essentially takes each three bytes from the array and
	 * converts them into four characters. This implementation, not using
	...
	private static String encodeBase64(byte[] bytes) {
		...
		while (i < 15) {
			...
			int d = (bytes[i++] & 0xff) << 16 | (bytes[i++] & 0xff) << 8
					| (bytes[i++] & 0xff);
		...
		// The last byte of the input gets put into two characters at the end of
		// the string.
	...
	/**
	 * Base64 decodes a short, 22-character UUID string (or 24-characters with
	 * padding) into a byte array. The resulting byte array contains 16 bytes.
	 * <p/>
	 * Base64 decoding essentially takes each four characters from the string
	 * and converts them into three bytes. This implementation, not using
	...
	private static byte[] decodeBase64(String s) {
		...
		while (i < 15) {
			...
			int d = I256[s.charAt(j++)] << 18 | I256[s.charAt(j++)] << 12
					| I256[s.charAt(j++)] << 6 | I256[s.charAt(j++)];
